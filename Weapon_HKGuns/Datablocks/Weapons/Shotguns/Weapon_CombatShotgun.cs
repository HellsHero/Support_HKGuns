datablock ItemData(HKGuns_CombatShotgunItem)
{
	...
	//gui stuff
	uiName = "HK Combat Shotgun";
	iconName = "Add-Ons/Weapon_Gun/icon_gun";
	doColorShift = false;
	colorShiftColor = "0.25 0.25 0.25 1.000";

	 // Dynamic properties defined by the scripts
	image = HKGuns_CombatShotgunImage;
	canDrop = true;
};

datablock ShapeBaseImageData(HKGuns_CombatShotgunImage)
{
    // Basic Item properties
    shapeFile = $hgunsk@"Models/CR.dts";
    emap = true;
    ...
    magSize = 8;                            /// Magazine size
    usesMag = 0;				            /// Shotgun no/yes
    chamberBullet = 0;                      /// Do bullets chamber?
    
    minSpread = 0.005;		    	        /// Initial spread
    spreadInc = 0.00045;		                /// Spread increase every shot (Stacks with projectile's 'recoil')
    spreadLoss = 0.0008;		    	    /// Spread lost every second when not firing
    spreadCeiling = 0.01;                   /// Maximum possible spread with this weapon
    movementSpread = 2/100;		            /// Percent of spread increased by movement
    
    firingRate = 385;                       /// Fastest the gun can fire, if you fire within firingRate*2 you will not regen spread (spreadLoss) during that firing cycle
    shotCount = 16;                         /// Not used if %player.overrideShells = true, this way it will use %player.shells for one firing cycle (sets .overrideShells to false and .shells to NULL after shooting)
    ...
    //Colourshift
    doColorShift = false;
    colorShiftColor = HKGuns_CombatShotgunItem.colorShiftColor; //"0.400 0.196 0 1.000";
	...
	//Reload State Tree
	stateName[3]                        = "singleReload"; //Shift Right
	stateSequence[3]                    = "reload";
	stateTransitionOnTriggerDown[3]     = "singleReloadDone";
	...
	stateName[8]                        = "reloadMisc"; //Shift Away, 200ms plant
	...
	stateTransitionOnTimeout[11]        = "fireAmmoCheckA";
	
	stateName[12]			            = "fireAmmoCheckA";
	stateScript[12]                     = "onAmmoCheck";
	stateTimeoutValue[12]               = 0.0001;
	stateTransitionOnTimeout[12]        = "fireAmmoCheckB";
	
	stateName[13]                       = "fireAmmoCheckB";
	stateTransitionOnAmmo[13]           = "delay";
	stateTransitionOnNoAmmo[13]         = "fireDelay";
	...
	stateName[15]                       = "fireDelay";
	stateTimeoutValue[15]               = 0.02;
	stateTransitionOnTimeout[15]        = "dryReady";
	
	stateName[16]                       = "dryReady";
	stateTransitionOnTriggerUp[16]      = "singleReload";
	stateTransitionOnTriggerDown[16]    = "dryFire";
	
	stateName[17]                       = "dryFire";
	stateTimeoutValue[17]               = 0.333;
	stateAllowImageChange[17]           = false;
	stateScript[17]                     = "onDryFire";
	stateWaitForTimeout[17]			    = true;
	stateTransitionOnTimeout[17]        = "fireDelay";
};

function reloadMisc_HKGuns_CombatShotgunImage(%this,%obj,%slot)
{
    //anims
...
function singleReload_HKGuns_CombatShotgunImage(%this,%obj,%slot)
{
    //anims
...
function fire_HKGuns_CombatShotgunImage(%this,%obj,%slot)
{
    //anims
    %obj.playThread(2,plant);
...
function dryfire_HKGuns_CombatShotgunImage(%this,%obj,%slot)
{
    //anims
    %obj.playThread(2,plant);
